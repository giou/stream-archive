...
class Monitor:
                ...
                if streams is not None:
                    ...
                    for channel in twitch_channels:
                        ...
                        if channel in self._live_channels and uid not in streams:
                            if not uid:
                                logger.warning("[monitor] stream without user_id, skipping stop")
                                continue
    ...
    async def _ensure_recording(
        ...
        """Start or restart the recording for a live channel."""
        async with self._lock_for(channel):
            already_live = channel in self._live_channels
            if already_live:
            ...
            # Compute the blocked reason and refresh the snapshot here,
            # under the lock. recorder.start() runs outside the lock below
    ...
    async def _ensure_stopped(self, channel: str, config: AppConfig) -> None:
        ...
        async with self._lock_for(channel):
            ...
            try:
                ...
                logger.error("[monitor] stop failed for %s, keeping live state", channel, exc_info=True)
                return
