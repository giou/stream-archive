...
def _emote_name(fragment_text, emote_id):
    m = _EMOTE_TOKEN_RE.match(fragment_text or "")
    return m.group(1) if m else str(emote_id)


def build_chat_root(channel, slug, title, started_wall, messages, duration_s=0.0):
...
async def fetch_emote_images(ids, client=None):
    ...
    async def one(eid):
        async with sem:
            try:
                resp = await client.get(EMOTE_URL.format(id=eid))
...
async def embed_kick_emotes(root, client=None):
